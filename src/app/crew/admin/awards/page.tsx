...
type Award = { id: number; name: string; description: string; imageurl: string; featured: number | null };
...
export default function AdminAwardsPage() {
  ...
  function openEdit(award: Award) {
    setSelected(award);
    setForm({ name: award.name, description: award.description, imageurl: award.imageurl, featured: award.featured === 1 });
    setFormError("");
    setDialogOpen(true);
  }
  ...
  return <CrewHeader><main className="flex-1 space-y-4">
    ...
    <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Award</TableHead><TableHead className="hidden md:table-cell">Description</TableHead><TableHead>Public</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
      {loading ? <TableRow><TableCell colSpan={4} className="py-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin" /></TableCell></TableRow> : awards.length === 0 ? <TableRow><TableCell colSpan={4} className="py-10 text-center text-muted-foreground">No awards found.</TableCell></TableRow> : awards.map((award) => <TableRow key={award.id}><TableCell><div className="flex items-center gap-3"><Image src={award.imageurl} alt="" width={40} height={40} unoptimized className="h-10 w-10 rounded-md border object-contain" /><span className="font-medium">{award.name}</span></div></TableCell><TableCell className="hidden max-w-lg md:table-cell"><p className="line-clamp-2 text-sm text-muted-foreground">{award.description}</p></TableCell><TableCell>{award.featured === 1 ? <Badge><Star className="mr-1 h-3 w-3" />Featured</Badge> : <span className="text-sm text-muted-foreground">—</span>}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="ghost" size="icon" onClick={() => openEdit(award)}><Edit className="h-4 w-4" /><span className="sr-only">Edit {award.name}</span></Button><AlertDialog><AlertDialogTrigger asChild><Button variant="ghost" size="icon" disabled={deletingId === award.id}><Trash2 className="h-4 w-4" /><span className="sr-only">Delete {award.name}</span></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete {award.name}?</AlertDialogTitle><AlertDialogDescription>This is allowed only when no aircraft or pilot grants reference the award.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => deleteAward(award)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div></TableCell></TableRow>)}
